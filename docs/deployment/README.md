...
이 디렉터리는 Docker Hub의 immutable backend/recap image pair를 Google Cloud의 두 non-production 환경으로 전달하는 절차를 설명한다. 저장소 변경이나 로컬 검증은 Google Cloud resource 생성, GitHub environment 설정, Vercel 연결, 실제 배포 성공을 증명하지 않는다. 외부 상태는 각 공급자의 authoritative read-back으로 별도 확인한다.
...
- [Private recap runtime and immutable release pair](recap-runtime.md)
