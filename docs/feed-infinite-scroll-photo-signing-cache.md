...
2026-09-18 로컬 검증에서 wishphoto 단위/통합, WishPhoto API/replay, FeedPhotoSigningCacheIT, FeedResultApiPostgresIT, SharedCardOpenApiDocumentationTest, OpenApiContractTest, OpenApiExamplesTest를 함께 실행하여 152개 모두 통과했다(실패·오류·건너뜀 0). 승인된 api/openapi.yaml의 SHA-256은 `f060a17c394678fa512d059be745be3f9d3991582d65168a0e2365cec1305adb`로 구현 전후 동일하다. 후속 전체 backend suite에서 기존 `RecapSuccessStoryApiIT`가 요청마다 새 URL을 기대하여 실패했던 검증을 승인된 재사용 계약에 맞춰 보완했다. Recap에서도 같은 URL과 원래 expiresAt을 보존하고, signer outage 중 잔여 수명이 정확히 30초이면 재사용하며 30초보다 1ns 부족하면 503을 반환한다. cold cache의 signer 실패도 503이고 recap generation 저장 결과는 변경하지 않는다.

보완 후 `./gradlew test --tests 'com.crabit.backend.api.RecapSuccessStoryApiIT' --console=plain`은 16개 모두 통과했다(실패·오류·건너뜀 0). 이어 `./gradlew test --console=plain`은 2분 27초에 성공했으며 139 suites, 총 754개 중 753개 통과, 실패·오류 0, 건너뜀 1이다. 건너뛴 `RecapInputParityIntegrationTest.actualSnapshotPythonHttpCoordinatorPersistenceAndOwnerRetrieval()`는 실제 로컬 Python receiver용 `CRABIT_RECAP_PARITY_CONFIG` 환경변수가 필요한 기존 선택적 테스트로, 이번 실행에서는 수행하지 않았다. `git diff --check`도 통과했고 승인된 OpenAPI 바이트는 그대로 유지했다. controller gate/commit/원격 read-back과 배포는 후속 controller 단계에서 별도로 확인한다.
