...
세 조회는 `(occurred_at DESC, event_id DESC)` keyset 순서를 사용한다. 커서는 operation, account, 선택적 Wish, 정렬 버전, 마지막 시각과 event ID를 함께 묶은 URL-safe 불투명 값이며 다른 리소스나 endpoint에서 재사용하면 거부한다. 계정 사용 가능 잔액 변화는 `ledger_event.account_delta - sum(ledger_wish_effect.wish_delta)`이고, 사건 직후 값은 같은 계정의 이 변화량을 오름차순으로 누적해 계산한다. Wish 사건 직후 금액도 같은 Wish effect를 오름차순 누적해 계산한다. V4 migration의 account-event, Wish-first effect, adjustment event-link index가 이 owner-scoped projection과 cursor 경계를 지원한다.
