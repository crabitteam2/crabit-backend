...
## Issue 49: 기간별 잔액 및 당시 대표 달성률

| 수용 기준 | 실행 검증 | 핵심 경계 |
| --- | --- | --- |
| C01 대표 교체 및 목표 수정 당시 달성률 | `HistoricalBalanceProjectionTest#usesTheSelectedHistoricalTargetAndRepresentativeForEveryBucket`, `HistoricalBalanceApiIT#replayFreezesValuesAndDigestAfterTargetAndRepresentativeMutation` | 30→50→40, 현재 목표로 과거 대체 금지 |
| C02 개설·수집 전 공백과 알려진 대표 부재 | `HistoricalBalanceProjectionTest#keepsPreOpeningAndPreCollectionUnknownSeparateFromRecordedRepresentativeAbsence`, `HistoricalBalanceRecordingIT#legacyMigrationCapturesActivationBaselineWithoutBackdatingLedgerApplicationTime` | UNKNOWN/null, 실제 activation baseline |
| C03 같은 시각과 이체의 원자적 순서 | `HistoricalBalanceRecordingIT#transactionFinalSnapshotsPreserveAtomicTransferAndHistoricalTargetChanges`, `HistoricalBalanceRecordingIT#waitingWriterUsesPostLockTimeAndMonotonicAccountRevision`, `HistoricalPeriodsTest#completedCalendarEndsExcludeExactApplicationAndCurrentPeriodsFreezeTheirHorizon` | post-lock 적용 시각, 제외 종료, 반쪽 이체 없음 |
| C04 성공 없음·성공 0·후속 실패·부족액 | `HistoricalBalanceProjectionTest#distinguishesNoSuccessZeroSuccessAndFailedLookupWithRealShortage`, `HistoricalBalanceRecordingIT#firstZeroUnchangedSuccessAndFailureAdvanceLookupWithoutInventingLedgerEvents` | 실제 관측 provenance와 내부 배정의 독립성 |
| 재생과 canonical 입력 | `HistoricalCanonicalFixtureTest#productionFinancialInputBuilderReproducesEveryApprovedGoldenDigest`, `HistoricalBalanceApiIT#replayFreezesValuesAndDigestAfterTargetAndRepresentativeMutation` | 고정 horizon/revision/digest, 현재 자격 재검사 |
