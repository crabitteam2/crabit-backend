...
public interface FriendshipRepository extends JpaRepository<Friendship, UUID> {
	...
	@Lock(LockModeType.PESSIMISTIC_WRITE)
	@Query("select friendship from Friendship friendship where friendship.studentLowId = :studentLowId and friendship.studentHighId = :studentHighId and friendship.endedAt is null order by friendship.academyId")
	List<Friendship> lockAllCurrentByPair(
			@Param("studentLowId") UUID studentLowId,
			@Param("studentHighId") UUID studentHighId);

	@Lock(LockModeType.PESSIMISTIC_WRITE)
	@Query("select friendship from Friendship friendship where friendship.academyId = :academyId and friendship.studentLowId = :studentLowId and friendship.studentHighId = :studentHighId")
	Optional<Friendship> lockByAcademyAndPair(
			@Param("academyId") UUID academyId,
			@Param("studentLowId") UUID studentLowId,
			@Param("studentHighId") UUID studentHighId);
}
